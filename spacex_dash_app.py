...
def get_pie_chart(selection):
    ...
    if selection == 'ALL':
        ...
        filtered_df = spacex_df[spacex_df['Launch Site'] == selection]
        filtered_df = filtered_df.groupby(['Launch Site', 'class']).size().reset_index(name='count')
        fig = px.pie(filtered_df, 
                     values='count', 
                     names='class', 
                     title=f'Total Success Launches for site {selection}')
    return fig
...
def get_scatter_chart(selection, payload_range):
    ...
    if selection == 'ALL':
        ...
        filtered_df = filtered_df[filtered_df['Launch Site'] == selection]
        fig = px.scatter(filtered_df, 
                         x='Payload Mass (kg)', 
                         y='class', 
                         color='Booster Version Category', 
